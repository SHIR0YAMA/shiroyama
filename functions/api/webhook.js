...
async function processNewFile(env, fileData) {
    ...
    if (from_chat_id.toString() !== env.CHANNEL_ID) {
        console.log(`Arquivo ignorado do chat ${from_chat_id}`);
        return;
    }
            ...
            if (text.startsWith('/start')) {
                ...
                if (payload && payload.startsWith('link_')) {
                    ...
                    if (!userToLink) {
                        ...
                        const telegramUsername = from_user.username || from_user.first_name;

                        const updateUserStmt = env.DB.prepare('UPDATE users SET telegram_chat_id = ?, telegram_username = ?, link_code = NULL WHERE id = ?');
                        await updateUserStmt.bind(chat_id, telegramUsername, userToLink.id).run();
                        
                        const successMessage = `✅ **Sucesso!** Sua conta do Telegram (${telegramUsername}) foi vinculada ao usuário \`${userToLink.username}\`.`;
                    ...
                    await sendMessage(env, chat_id, `👋 Olá, ${from_user.first_name || 'Usuário'}! Use o site para interagir.`);
                }
