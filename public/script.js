...
function buildFileTree(files) {
    ...
    files.forEach(file => {
        ...
        parts.forEach((part, index) => {
            if (index === parts.length - 1) { currentLevel[part] = { ...file, _isFile: true }; } else { if (!currentLevel[part]) { currentLevel[part] = {}; } currentLevel = currentLevel[part]; }
        });
    });
    return tree;
}
...
async function renderProfilePage() {
    ...
    try {
        ...
        if (userData.telegram_chat_id) {
            const displayUsername = userData.telegram_username ? (userData.telegram_username.includes('@') ? userData.telegram_username : `@${userData.telegram_username}`) : (userData.telegram_display_name || 'N/A');
            telegramSectionHTML = `<h3>Conta do Telegram Vinculada</h3><p>Usuário: <strong>${displayUsername}</strong></p><p>Chat ID: <strong>${userData.telegram_chat_id}</strong></p><button id="unlink-btn">Desvincular Conta</button>`;
        } else {
        ...
        if (userData.telegram_chat_id) {
            document.getElementById('unlink-btn').onclick = async () => {
                if (confirm('Tem certeza?')) { try { await apiCall('user/unlink-telegram', 'POST'); showNotification('Conta desvinculada com sucesso.', 'success'); router(); } catch (error) { showNotification(`Erro: ${error.message}`, 'error'); } }
            };
        } else {
