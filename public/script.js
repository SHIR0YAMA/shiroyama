...
function logout() {
    state.token = null; state.username = null; state.role = null; state.permissions = [];
    localStorage.clear();
...
function buildFileTree(files) {
    const tree = {};
    // Primeiro, cria todas as pastas a partir dos placeholders
    files.forEach(file => {
    ...
    // Depois, adiciona todos os arquivos, ignorando os placeholders
    files.forEach(file => {
        if (file.name.endsWith('/.placeholder')) return;
...
function renderLoginPage() {
    ...
    document.getElementById('login-form').onsubmit = async (e) => { e.preventDefault(); try { const data = await apiCall('auth/login', 'POST', { username: e.target.username.value, password: e.target.password.value }); login(data.token); window.location.hash = '/'; } catch (error) { showNotification(`Erro no login: ${error.message}`, 'error'); } };
}
...
function renderRegisterPage() {
    ...
    document.getElementById('register-form').onsubmit = async (e) => { e.preventDefault(); try { const data = await apiCall('auth/register', 'POST', { username: e.target.username.value, password: e.target.password.value }); showNotification(data.message, 'success'); window.location.hash = '/login'; } catch (error) { showNotification(`Erro no registro: ${error.message}`, 'error'); } };
}
...
async function renderProfilePage() {
    ...
    try {
        ...
        if (userData.telegram_chat_id) {
            document.getElementById('unlink-btn').onclick = async () => { if (confirm('Tem certeza?')) { try { await apiCall('user/unlink-telegram', 'POST'); showNotification('Conta desvinculada com sucesso.', 'success'); router(); } catch (error) { showNotification(`Erro: ${error.message}`, 'error'); } } };
        } else {
            document.getElementById('link-telegram-btn').onclick = (e) => { const linkButton = e.target; linkButton.disabled = true; linkButton.textContent = 'Gerando...'; const randomCode = Math.random().toString(36).substring(2, 8).toUpperCase(); const linkCodeWithPrefix = `link_${randomCode}`; apiCall('user/prepare-link-code', 'POST', { linkCode: linkCodeWithPrefix }).then(() => { window.open(`https://t.me/ShiroyamaBot?start=${linkCodeWithPrefix}`, '_blank'); linkButton.textContent = 'Verifique o Telegram!'; showNotification('Conclua o vínculo no Telegram.', 'info'); startFaviconBlink(); setTimeout(() => router(), 15000); }).catch(err => { showNotification(`Erro: ${err.message}`, 'error'); linkButton.disabled = false; linkButton.textContent = 'Vincular com o Telegram'; }); };
            document.getElementById('why-link-q').onclick = (e) => { e.preventDefault(); whyLinkModal.classList.add('show'); };
        }
        document.getElementById('password-form').onsubmit = async (e) => { e.preventDefault(); const currentPassword = e.target['current-password'].value; const newPassword = e.target['new-password'].value; if (newPassword !== e.target['confirm-password'].value) { showNotification("As senhas não coincidem.", 'error'); return; } try { const data = await apiCall('auth/change-password', 'POST', { currentPassword, newPassword }); showNotification(data.message, 'success'); logout(); } catch (error) { showNotification(`Erro: ${error.message}`, 'error'); } };
    } catch (error) {
...
function renderFilesPage(path) {
    ...
    const items = Object.entries(content).sort(([nameA, itemA], [nameB, itemB]) => {
        const isFileA = itemA._isFile; const isFileB = itemB._isFile; if (isFileA && !isFileB) return 1; if (!isFileA && isFileB) return -1; const sortOrder = state.sort.order === 'asc' ? 1 : -1; if (state.sort.key === 'name') { return nameA.localeCompare(nameB, undefined, { numeric: true }) * sortOrder; } if (state.sort.key === 'size') { return (itemA.file_size || 0) - (itemB.file_size || 0) * sortOrder; } return 0;
    });
    if (items.length === 0) {
...
async function router(forceRoute) {
    ...
    if (route === 'admin' && !hasPermission('can_manage_users') && !hasPermission('can_manage_roles')) { showNotification("Acesso negado.", "error"); window.location.hash = '/'; return; }
    if (route === 'profile' && !state.token) { window.location.hash = '/login'; return; }
    ...
    switch (route) {
        ...
        case 'admin': renderAdminPage(path[1]); break;
        case 'profile': renderProfilePage(); break;
        default: if (state.token) { if (hasPermission('can_view_files')) { renderFilesPage(path); } else { mainContent.innerHTML = "<h2>Acesso Negado</h2><p>Você não tem permissão para visualizar arquivos.</p>"; } } else { renderLoginPage(); } break;
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    mainContent.addEventListener('click', (e) => {
        ...
        if (target.classList.contains('btn-delete')) { const isFolder = target.dataset.isfolder === 'true'; const key = target.dataset.key; deleteItems([key], isFolder, isFolder ? key.split('/').pop() : ''); }
        if (target.id === 'create-folder-btn') { openCreateFolderModal(false); }
        if (target.classList.contains('sortable-header')) { const sortKey = target.dataset.sort; if (state.sort.key === sortKey) { state.sort.order = state.sort.order === 'asc' ? 'desc' : 'asc'; } else { state.sort.key = sortKey; state.sort.order = 'asc'; } router(); }
        ...
        if (target.classList.contains('save-user-role-btn')) {
            ...
            apiCall('admin/users/update-role', 'POST', { userId: parseInt(userId), newRoleId: parseInt(newRoleId) }).then(() => showNotification("Cargo do usuário atualizado.", "success")).catch(err => showNotification(`Erro: ${err.message}`, "error"));
        }
        if (target.classList.contains('delete-user-btn')) {
            const userId = target.dataset.id;
            if (confirm('Tem certeza que deseja excluir este usuário?')) {
                apiCall('admin/delete-user', 'POST', { userId: parseInt(userId) }).then(() => { showNotification("Usuário excluído.", "success"); router('admin/users'); }).catch(err => showNotification(`Erro: ${err.message}`, "error"));
            }
        }
        ...
        if (target.classList.contains('delete-role-btn')) {
            ...
            if (confirm('Tem certeza que deseja excluir este cargo?')) {
                apiCall(`admin/roles/${roleId}`, 'DELETE').then(() => { showNotification("Cargo excluído.", "success"); router('admin/roles'); }).catch(err => showNotification(`Erro: ${err.message}`, "error"));
            }
        }
    });
