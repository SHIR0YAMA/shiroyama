...
function renderFilesPage(path) {
    ...
    items.forEach(([name, item]) => {
        const tr = document.createElement('tr');
        if (item._isFile) {
            tr.innerHTML = `<td><input type="checkbox" class="file-checkbox" data-message-id="${item.message_id}" data-key="${item.name}"></td> <td class="file-item-name"><span>📄</span> <span>${name}</span></td> <td class="size-col">${formatFileSize(item.file_size)}</td> <td class="download-col"><button class="btn-single-forward" data-message-id="${item.message_id}" title="Receber no Telegram">Receber</button></td>`;
        } else {
